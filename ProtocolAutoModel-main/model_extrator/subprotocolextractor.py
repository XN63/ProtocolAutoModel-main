...
def build_budgeted_packet(sections_path:str, 
                          sentences_path:str,
                          llm_config:LLMConfig, 
                          model="gpt-4o")->Dict[str,Any]:
    # 获取模型特定的预算参数
    ...
    keep_per_sec = model_config.keep_per_section
...
def main():
    ...
    out_dir = os.path.join(os.getcwd(), "output", "agents"); 
    ...
    for i,pkt in enumerate(shards, start=1):
        ...
        if tot > HARD_CAP:
            ...
            while ev and messages_tokens(
                llm.format_messages(SYSTEM_PROMPT, 
                                    DEVELOPER_PROMPT, 
                                    build_user_prompt({**safe,"evidence":ev})
                                    ), 
                                    MODEL
                                ) > HARD_CAP:
    ...
    #判断drafts长度：大于1才进行合并，等于1则直接保存，为0则报错
    if len(drafts) == 0:
        logging.error("没有收到任何草案，无法合并。请检查前面的日志以了解详情。")
    elif len(drafts) == 1:
        out_path = os.path.join(out_dir, f"QUIC_subprotocols_{MODEL}_developpromptchange.json")
        save_json(json.loads(drafts[0]), out_path)
        # print("===== 子协议提取（单片结果） =====")
        # print(drafts[0])
        logging.info(f"仅有一片草案，无需合并。已保存：{out_path}")
        return  None
    else:
        logging.info(f"收到 {len(drafts)} 片草案，准备合并。")
        # === 4) 合并草案 ===
        merged = merge_drafts(drafts)
        logging.info(f"修改developer prompt:添加对cites的一致性约束。")
        out_path = os.path.join(out_dir, f"QUIC_subprotocols_{MODEL}_developpromptchange.json")
